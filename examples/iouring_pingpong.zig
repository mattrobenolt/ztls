            ...
            switch (ev) {
                .write => |w| {
                    ...
                    if (try hs.sendServerFlightBuffered(&flight)) |flight_bytes| {
                        try sendAll(&ring, conn.stream.handle, flight_bytes);
                        hs.completeWrite();
                    }
                },
                .none => {},
        ...
        while (try rb.next()) |record| {
            ...
            switch (ev) {
                .application_data => |data| {
                    ...
                    try sendAll(&ring, conn.stream.handle, rec);
                    hs.completeWrite();
                    round += 1;
                },
                .write => |w| {
                ...
                .closed => {
                    ...
                    try sendAll(&ring, conn.stream.handle, close);
                    hs.completeWrite();
                    return;
                },
                .none => {},
...
fn clientRun(client_keypair: ztls.x25519.KeyPair, actual_port: u16) !void {
    ...
    try sendAll(&ring, stream.handle, first);
    hs.completeWrite();
    ...
    while (true) {
        ...
        while (try rb.next()) |record| switch (try hs.handleRecord(record, &out.buffer)) {
            .application_data => |data| {
                ...
                if (round == rounds) {
                    try sendAll(&ring, stream.handle, try hs.sendAlert(.close_notify, &out.buffer));
                    hs.completeWrite();
                    return;
                }
                round += 1;
                const rec = try hs.sendApplicationData(ping(&msg_buf, round), &out.buffer);
                try sendAll(&ring, stream.handle, rec);
                hs.completeWrite();
            },
            .write => |w| {
                try sendAll(&ring, stream.handle, w);
