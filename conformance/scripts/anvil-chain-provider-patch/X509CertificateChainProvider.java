...
public class X509CertificateChainProvider {
    ...
    // ztls conformance patch (#91): RFC 5280 §4.2.1.9 requires every CA
    // certificate to carry a critical basicConstraints extension with cA=true.
    // Upstream v1.5.0 builds the chain's self-signed signing certificate with
    // only subject/publicKeyType/signatureAlgorithm, so every generated root
    // is a non-conforming CA certificate. Stamp the constraint on the signing
    // cert; leaves and all other config diversity are untouched.
    private static void addCaBasicConstraints(X509CertificateConfig signingCert) {
        ...
        basicConstraints.setIncludeCA(DefaultEncodingRule.ENCODE);
        signingCert.addExtensions(basicConstraints);
    }
